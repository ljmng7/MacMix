...
| 能力 | 说明 |
| --- | --- |
| 系统音量 | 直接在菜单栏调节当前输出设备的系统音量，并为兼容的外接显示器提供 DDC/CI 后备控制。 |
| 输出设备 | 在扬声器、耳机、AirPods、外接显示器等输出设备之间快速切换。 |
| 输入设备 | 在同一面板中切换麦克风并调节输入音量。 |
| 单应用混音 | 在应用正在输出音频时，单独调低音量，或按需提升到最高 200%。 |
...
- 200% 单应用音量增强是可选功能，可在设置中开启。
- 外接显示器音量后备控制需要显示器在当前线材或扩展坞上支持 DDC/CI 扬声器音量（VCP `0x62`）。DisplayLink、部分 HDMI 链路、电视，或在 OSD 菜单中关闭了 DDC/CI 的显示器可能无法使用。
